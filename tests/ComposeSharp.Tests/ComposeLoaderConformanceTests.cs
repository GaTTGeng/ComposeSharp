...
public sealed class ComposeLoaderConformanceTests
{
    public static IEnumerable<object[]> Fixtures()
    {
        ...
        yield return ["command-environment"];
        yield return ["network-storage"];
        yield return ["service-lifecycle"];
    ...
    public void Load_Fixture_MapsExpectedComposeFields(string scenario)
    {
        ...
        if (File.Exists(dotEnvTemplate))
            File.Copy(dotEnvTemplate, Path.Combine(fixtureDirectory, ".env"), overwrite: true);

        var project = LoadFixture(scenario, fixtureDirectory, composeFile);
        Assert.Equal(scenario, Path.GetFileName(project.WorkingDirectory));

        AssertFixture(scenario, composeFile, () =>
        {
            var app = Assert.Single(project.Services);
            switch (scenario)
            {
                case "build-definition":
                    Assert.Equal("example/app:build", app.Image);
                ...
                case "command-environment":
                    ...
                    Assert.Equal(["dotnet", "app.dll"], app.Command);
                ...
                default:
                    throw new InvalidOperationException($"No assertion is defined for fixture '{scenario}'.");
            }
        });
    }
